...
export default function AuthPage() {
  ...
  const sendOtp = async (fullPhone) => {
    try {
      ...
      if (data.success) {
        ...
        setMessage(`Could not send OTP! Try again in few minutes..`);
      }
    } catch (err) {
  ...
  const sendEmailOtp = async () => {
    ...
    try {
      const res = await fetch(`${BaseUrl}/api/otp/email/send-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.token) {
        ...
        setMessage("Could not send Email OTP ❌");
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const verifyEmailOtp = async () => {
    try {
      const res = await fetch(`${BaseUrl}/api/otp/email/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (data.message === "Email OTP verified successfully") {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin && (!otpVerified || !emailOtpVerified)) {
      setMessage("Please verify phone or email before signing up ❌");
      return;
    }
    ...
    if (!data.err) {
      setErrors({});
      if (isLogin) {
        const resp = await fetch(`${BaseUrl}/api/auth/verify`, {
          credentials: "include",
        });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-100 to-gray-200 px-4">
        <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-lg">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={!isLogin ? !otpVerified || !emailOtpVerified : false}
              className={`w-full py-3 rounded-xl font-semibold transition ${
                (!isLogin && !emailOtpVerified) ||
                (submitted && validate().length > 0)
            ...
            >
              {isLogin
                ...
                : `${submitted ? "Signing you up" : "Sign Up"}`}
            </button>
          </form>
