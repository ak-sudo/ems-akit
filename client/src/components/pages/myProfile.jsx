    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get(
          `${BaseUrl}/api/profile/${data.role}/${data.id}`,
        );
        setUser(res.data);
      ...
      <div className="flex justify-center items-center py-10 px-6 mt-8 bg-gray-100 min-h-screen">
        <div className="bg-white shadow-lg rounded-2xl p-8 max-w-lg w-full border border-gray-200">
          ...
          <div className="flex justify-evenly mb-6">
            <img
              src={user.data.dpurl || "https://freesvg.org/img/user-icon.png"}
              alt="Profile"
          ...
          <p className="text-center text-gray-500 mb-6 capitalize">
            { user.response && user.response.designation || "Student/Faculty"}
          </p>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="p-4 bg-gray-50 rounded-xl border">
              ...
              <p className="text-gray-800 font-medium">{user.data.email}</p>
            </div>
            <div className="p-4 bg-gray-50 rounded-xl border">
              <p className="text-gray-500 text-sm">Phone</p>
              <p className="text-gray-800 font-medium">{user.data.phone}</p>
            </div>
          </div>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-xl border">
                ...
                {user.response ? (
                    Object.entries(user.response).map(([key, value]) => (
